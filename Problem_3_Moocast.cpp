...
int dfs(int v, vector<bool> &visited,vector<vector<bool>> connected)
{
    int ans=1;
    visited[v]=true;
    for(int u=0;u<connected.size();u++)
    {
        if(!visited[u] && connected[v][u])
        {
            ans+=dfs(u,visited,connected);
        }
    }
    return ans;
}
...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        ans=max(ans,dfs(i,visited,connected));
    }
    cout<<ans<<endl;
}
