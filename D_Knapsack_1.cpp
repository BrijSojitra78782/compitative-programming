...
int func(int n,int weight,vector<int> &w,vector<int> &v)
{
    if(n==0 || weight==0)
    {
        return 0;
    ...
    if(w[n-1]<=weight)
    {
        return dp[n][weight]=max(v[n-1]+func(n-1,weight-w[n-1],w,v),func(n-1,weight,w,v));
    }
    else
    {
        return dp[n][weight]=func(n-1,weight,w,v);
    }
}
